...
export default function LanguageSelector({ isScrolled }: LanguageSelectorProps) {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="min-w-[140px] bg-cream border-sage/30 shadow-card">
        {LANGUAGES.map((lang) => (
          <DropdownMenuItem
            key={lang.code}
          ...
          >
            ...
            {language === lang.code && <Check className="w-3.5 h-3.5 text-moss flex-shrink-0" />}
          </DropdownMenuItem>
        ))}
      </DropdownMenuContent>
    </DropdownMenu>
